...
class NotificationService {
  ...
  static Future<void> onActionReceivedMethod(ReceivedAction receivedAction) async {
    ...
    if(receivedAction.buttonKeyPressed == "routeToFINKI"){
      LocationService().openGoogleMaps();
    }
  }

  // test scheduled notification with location - notification pops up shortly after method is called
  static Future showSimpleNotification() async {
    Exam exam = Exam(course: "Test",timestamp: DateTime.now().add(const Duration(seconds: 2)));
    ...
    AwesomeNotifications().createNotification(
      content: NotificationContent(
          ...
          channelKey: 'high_importance_channel',
          title: "Exam reminder",
          body: "You have an upcoming exam for ${exam.course} tomorrow at ${DateFormat('HH:mm').format(exam.timestamp)}. Plan your route to arrive on time"
      ),
      ...
      actionButtons: [
        NotificationActionButton(key: "routeToFINKI", label: "Open Google Maps"),
  ...
  static Future showScheduledNotificationDayBefore(String course, DateTime examTime) async {
    ...
    AwesomeNotifications().createNotification(
        content: NotificationContent(
            ...
            channelKey: 'high_importance_channel',
            title: "Exam reminder",
            body: "You have an upcoming exam for $course tomorrow at ${DateFormat('HH:mm').format(examTime)}. Plan your route so you arrive on time."
        ),
        schedule: NotificationCalendar(
            year: reminderTime.year,
            month: reminderTime.month,
